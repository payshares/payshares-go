...
func TestHandler(t *testing.T) {
    ...
    INSERT INTO people (id, name, domain) VALUES 
      ('GD2GJPL3UOK5LX7TWXOACK2ZPWPFSLBNKL3GTGH6BLBNISK4BGWMFBBG', 'scott', 'payshares.org'),
      ('GCYMGWPZ6NC2U7SO6SMXOP5ZLXOEC5SYPKITDMVEONLCHFSCCQR2J4S3', 'bartek', 'payshares.org');
  `)
	defer db.Close()
	...
	server.GET("/federation").
		WithQuery("type", "name").
		WithQuery("q", "scott*payshares.org").
		Expect().
	...
	server.GET("/federation").
		...
		Status(http.StatusOK).
		JSON().Object().
		ContainsKey("payshares_address").
		ValueEqual("payshares_address", "scott*payshares.org")
