...
import (
	...
	"github.com/payshares/go/crc16"
	"github.com/payshares/go/support/errors"
)

// ErrInvalidVersionByte is returned when the version byte from a provided
// psrkey-encoded string is not one of the valid values.
var ErrInvalidVersionByte = errors.New("invalid version byte")

// VersionByte represents one of the possible prefix values for a PsrKey base
// string--the string the when encoded using base32 yields a final PsrKey.
...
const (
	...
	//VersionByteHashTx is the version byte used for encoded payshares hashTx
	//signer keys.
	VersionByteHashTx = 19 << 3 // Base32-encodes to 'T...'

	//VersionByteHashX is the version byte used for encoded payshares hashX
	//signer keys.
	VersionByteHashX = 23 << 3 // Base32-encodes to 'X...'
)

// Decode decodes the provided PsrKey into a raw value, checking the checksum
...
// Encode encodes the provided data to a PsrKey, using the provided version
// byte.
...
// potentially be psrkey encoded (i.e. it has both a version byte and a
