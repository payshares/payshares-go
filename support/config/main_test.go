...
func TestAccountIDValidator(t *testing.T) {
	var val struct {
		Empty        string `valid:"payshares_accountid"`
		NotPSRKey    string `valid:"payshares_accountid"`
		NotAccountID string `valid:"payshares_accountid"`
		Valid        string `valid:"payshares_accountid"`
		WrongType    int    `valid:"payshares_accountid"`
	}
	...
	_, ok = fields["NotPSRKey"]
	assert.True(t, ok, "NotPSRKey is not an invalid field")
...
func TestSeedValidator(t *testing.T) {
	var val struct {
		Empty     string `valid:"payshares_seed"`
		NotPSRKey string `valid:"payshares_seed"`
		NotSeed   string `valid:"payshares_seed"`
		Valid     string `valid:"payshares_seed"`
		WrongType int    `valid:"payshares_seed"`
	}
